...
public class GameManager : MonoBehaviour
{
    ...
    public async void GameOver()
    {
        ...
        //プレイヤーの子オブジェクトを非表示
        await DeactivatePlayerProps();

        //ゲームオーバー時に消したいCanvasを非表示
        await DeactivateGameOverCanvas();

        //スコア(生存時間)を送信
        await _RankingManager.SubmitScore();
    ...
    //ゲームオーバー時にプレイヤーの武器やインディケーターを消す
    private async Task DeactivatePlayerProps()
    {
        foreach(Transform prop in _Player.PlayerHaveObjectsParent)
        {
    ...
    //ゲームオーバー時に消したいCanvasを非表示
    private async Task DeactivateGameOverCanvas()
    {
        for (int i = 0; i < gameOverNonActiveCanvasList.Count; i++)
        {
            var canvas = gameOverNonActiveCanvasList[i].gameObject;
    ...
    {
        BGMAudio.Instance.BGMAudioSource.volume = 0.3f * value * 2;
    ...
    {
        SEAudio.Instance.SEAudioSource.volume = 0.6f * value * 2;
